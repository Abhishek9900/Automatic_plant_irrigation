...
void setup() {
  ...
  port = new Serial(this, "COM3", 9600); // initializing the object by assigning a port and baud rate (must match that of Arduino)
...
void draw() 
{
  ...
  { 
    ...
    if (serial != null) {
      ...
      println(a[0]); //print moisture value
      println(a[1]); //print temperature value
      println(a[2]); //print humidity value
      function();
    }
  }
}
...
void function()
{
  if ( msql.connect() )
    {
        msql.query( "insert into iot(MOISTURE, TEMPERATURE, HUMIDITY)values("+a[0]+","+a[1]+","+a[2]+")" );
    }
    else
    {
        // connection failed !
